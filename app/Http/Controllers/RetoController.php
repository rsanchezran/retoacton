...
class RetoController extends Controller
{
    ...
    public function dia(Request $request, $dia, $genero, $objetivo)
    {
        ...
        if ($user->num_inscripciones > 1) {
            $teorico = $diasRetoOriginal + (($user->num_inscripciones - 2) * $diasReto) + (Carbon::now()->startOfDay()->diffInDays($inicioReto)+1);
        ...
        if ($usuarioDieta == null) {
            ...
            error_log('hhdkjhjkashdkjhaskjdhkjashdkjhaskjdhkjahsdkjhaskdj');
            error_log($objetivo);
...
//            return view('reto.dia', ['dia' => $dia, 'genero' => $genero, 'objetivo' => $objetivo, 'lugar' => $user->modo,
  //              'dias' => $user->dias,  'semana' => $semana, 'diasReto' => $diasReto]);
        } else {
            ...
            if ($dietaCreada==0){
                ...
                app('App\Http\Controllers\HomeController')->generarDieta($request->user(), $objetivo, $peso, $alimentosIgnorados, $numDietas);
            }
            $diasTranscurridosuno = (Carbon::now()->startOfDay()->diffInDays($inicioReto)+1);
